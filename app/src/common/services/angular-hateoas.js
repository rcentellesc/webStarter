 ...
 */
	...
	app.provider("HateoasInterface", function () {

		// global Hateoas settings
		var globalHttpMethods;
		var linksHALKey = "links";
		var linksResourceKey="linksResource";
		...
		/*	setLinksKey: function (newLinksKey) {
			...
			$get: ["$injector", function ($injector) {
				...
				var resource = function (linkName, bindings, httpMethods) {
					if (linkName in this[linksResourceKey]) {
						return $injector.get("$resource")(this[linksResourceKey][linkName], bindings, httpMethods || globalHttpMethods);
					} else {
				...
				var HateoasInterface = function (data) {
					...
					if (data[linksHALKey]) {
						var links = {};
						links[linksResourceKey] = arrayToObject("rel", "href", data[linksHALKey]);
					...
					angular.forEach(data, function (value, key) {
						if (key !== linksHALKey && key!==linksResourceKey && angular.isObject(value) && (angular.isArray(value) || value[linksHALKey])) {
	...
	app.provider("HateoasInterceptor", ["$httpProvider", function ($httpProvider) {
